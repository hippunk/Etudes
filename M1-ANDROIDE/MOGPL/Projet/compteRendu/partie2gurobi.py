...
for i in range(N):
    n.addConstr(quicksum(x[i][j]*u[i][j] for j in 
			range(M))-y>=0,"contrainte%d" % i)
    
for j in range(M):
    n.addConstr(quicksum(x[i][j] for i in 
			range(N))==1,"contrainte%d" % (N+j))
...
print ""
print 'Valeur de la fonction objectif :', n.objVal
