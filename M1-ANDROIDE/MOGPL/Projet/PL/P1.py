...
for i in range(N):
    m.addConstr(quicksum(x[i][j]*u[i][j] for j in range(M))-y>=0,"contrainte%d" % i)
    
for i in range(N):
    m.addConstr(quicksum(x[i][j] for j in range(M))==1,"contrainte%d" % (N+i))    
    
for j in range(M):
    m.addConstr(quicksum(x[i][j] for i in range(N))==1,"contrainte%d" % (2*N+j))
