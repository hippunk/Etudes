...
def iteration (x, y,w,b,epsilon, iteration):
    ...
    for i in range (iteration):
        #print epsilon
        f = 1./(1+np.exp(-X.dot(wj)-bresultat))
        #print wj
        wj = wj + epsilon*(x * (y - f).reshape(N,1)).sum(0)
        #print wj
        bresultat = bresultat + epsilon * (y - f).reshape(N,1).sum(0)
        
        stock[i]= (y * np.log(f) + (1-y)*np.log(1-f)).sum()
        #print stock[i]
...
#
#w,b,epsilon = initialisation(0.001,len(X[0]))
...
w,b,epsilon = initialisation(0.001,len(X[0]))
w ,b , liste = iteration (Xd,Yc0, w,b,epsilon,100)
p = plt.figure()
p = plt.plot (liste)
plt.show()
...
thetaRL = learn(X,Y,10, 0.0004, 100)
